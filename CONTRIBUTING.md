# Contributing to SEQUENTICHAIN ($SQX)

Thank you for contributing to **sequentichain** — the **L2 Sequencing / Rollup** pillar of the [AXIOLEDGER](https://github.com/axioledger) ecosystem.
...
All repositories follow the `sqx-*` prefix convention:
- Example repos: `sqx-rollup-core, sqx-sequencer`

Branch naming:
| Prefix | Use |
|---|---|
| `feat/sqx-*` | New feature |
| `fix/sqx-*` | Bug fix |
| `security/sqx-*` | Security patch — requires Security Auditor approval |
| `docs/sqx-*` | Documentation update |

Always branch from `dev`:
```bash
git checkout dev && git pull upstream dev
git checkout -b feat/sqx-your-feature
```
...
**TypeScript/Node.js (`@sequentichain/*` packages):**
```bash
npm test
